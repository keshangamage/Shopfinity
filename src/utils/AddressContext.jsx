...
export const AddressProvider = ({ children }) => {
  ...
  const [addresses, setAddresses] = useState(() => {
    try {
      const savedAddresses = localStorage.getItem(`shopfinity_addresses_${userId}`);
  ...
  const [defaultAddressId, setDefaultAddressId] = useState(() => {
    try {
      const savedDefaultId = localStorage.getItem(`shopfinity_default_address_${userId}`);
  ...
  useEffect(() => {
    try {
      if (userId) {
        localStorage.setItem(`shopfinity_addresses_${userId}`, JSON.stringify(addresses));
      }
    } catch (error) {
  ...
  useEffect(() => {
    try {
      if (userId && defaultAddressId) {
        localStorage.setItem(`shopfinity_default_address_${userId}`, defaultAddressId);
      }
    } catch (error) {
